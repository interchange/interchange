# Vend::Payment::Protx - Interchange Protx support
#
# Protx.pm, v 2.1.2, July 2007
#
...
=head1 Interchange Protx Support

Vend::Payment::Protx $Revision: 1.1 $
...
=head1 This package is for the 'Protx Direct' payment system.
...
1 Place this module in <IC_root>/lib/Vend/Payment/Protx210.pm

2 Call it in interchange.cfg with:
    Require module Vend::Payment::Protx210
...
3 Add into variable.txt (tab separated):
  ...
  Add a new route into catalog.cfg (options for the last entry in parentheses):
    ...
    Route protx logdir "/path/to/your/shop/tmp"
...
payment safeguard record, set either:
	Route protx logdir "/full/path/to/your/catalogue/tmp", or
	<input type=hidden name=logdir value='your_choice_here'>
into your payment page. This allows an individual user to have his own logs in a shared
hosting environment. If you use this, ensure that you add a 'logdir' field to
transactions.txt. If not set, defaults to the system /tmp.
...
be NULL. If MySQL does not try to write to a field that is marked NOT NULL then it will fail silently.
...
If using BDB/GDBM there is one known issue relating to SQL syntax - it is highly recommended that you
use an SQL database such as MySQL, Postgres, or DB2. This is now compulsory for recent versions of IC.
...
my $logdir = $::Values->{logdir} || charge_param('logdir') || '/tmp';
...
my $startDate;
if (!$startDateMonth) { $startDate = '';
      }
else { $startDate = sprintf '%02d%02d', $startDateMonth, $startDateYear;
}
...
#::logDebug("Sent to Protx: $key=$query{$key}");
 }
...
if (($available eq 'yes') and ($amount > 0) and ($::Values->{mv_order_route} !~ /ptipm_route|protx_vt_route/i)) {
 ...
 # $in = 'test'; ######################## testing only, will force offline mode
   if ($in =~ /^200 OK$/)  { $request = 'psp'; }
   else { $request = 'offline'; }
