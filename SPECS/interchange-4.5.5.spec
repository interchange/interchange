...
perl Makefile.PL \
	...
	PREFIX=$RPM_BUILD_ROOT/usr/local/interchange \
...
cd $RBR/usr/local/interchange
export PERL5LIB=$RBR/usr/local/interchange/lib
export MINIVEND_ROOT=$RBR/usr/local/interchange
...
# Create a interchange user. Do not report any problems if it already
# exists. We do it first so it won't error on chmod
useradd -M -r -d /var/lib/interchange -s /bin/bash -c "Interchange server" interchange 2> /dev/null || true 
...
/var/lib/interchange/barry
/var/lib/interchange/simple
/usr/local/bin/interchange
...
if test "x$status" != x1
then
	rm -f /usr/local/interchange/_*storable
fi
