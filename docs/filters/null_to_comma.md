...
Replaces NUL characters in the value with a comma (`,`).

## Syntax

    [value name=field filter="null_to_comma"]
...
ASCII NUL (`\0`) characters between them. The `null_to_comma` filter
converts each run of NUL into a single comma, producing a comma-separated
scalar suitable for storing in one database column.
...
joined internally as `"S\0M\0L"`, applying the filter:
    ...
    [value name=sizes filter="null_to_comma"]
...
[null_to_colons](null_to_colons.md), [null_to_space](null_to_space.md),
[colons_to_null](colons_to_null.md)

## Source

Defined in `code/Filter/null_to_comma.filter`.
