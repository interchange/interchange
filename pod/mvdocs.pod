...
Interchange -- shopping cart and electronic catalog system

=head2 Version

This document describes Interchange 4.5.x, based on Minivend 4.0.
...
is completed, Interchange submits the order to the system via email or an
external order entry program.

Interchange is a high-end, fully customizable, powerful software system with
...
Interchange plugs into a system with an SSL (Secure Sockets Layer) server,
...
Many different catalogs can be run from the same Interchange server,
allowing an ISP to serve many different customers from one or just
a few Interchange server processes. As many as 2,000 Interchange catalogs have
been run on one machine from the same server process.

Interchange is powerful, and correspondingly complex. It can easily handle
...
you only have a few items to catalog, Interchange is probably overkill for
...
=head1 OVERVIEW OF INTERCHANGE

The following section describes how Interchange works, and should be read
by anyone creating a Interchange catalog.

=head2 The Vend Concept

Interchange is a descendent of Vend, originally developed by Andrew Wilcox in
1995, then greatly expanded and enhanced in the form of Minivend from Mike Heins.
Though the original Vend was much simpler than Interchange in
implementation, the basic concept remains unchanged. Quite simply, Interchange
maintains its own set of pages, I<outside of regular HTML space>, which contain
special tags that are interpreted by Interchange.

The tags, which are in C<[square brackets]>, are interpreted by Interchange and
...
Interchange remembers input by a user from form to form, and the value of any
...
Interchange can have an unlimited number of attached databases, either in one of
...
Interchange has a powerful object system that allows direct access
to Perl and external programs. There is an ASP-like syntax which
can be used, or the traditional Interchange tag approach can be employed.
...
Interchange supports different search engines, including I<Glimpse>, or you can
make your own SQL database queries and have Interchange process the output for you.
...
to the Interchange CGI link program (generically called VLINK or TLINK, more on
that below).  The Interchange server is already running on your system, and your
catalog has been designed and tested.

The link, which is a regular CGI program, calls the Interchange server
through a socket.  The Interchange server sees the path information which
...
The user clicks on the link and Interchange looks in the I<products> database,
...
Interchange keeps track of who is ordering by one of two means:

=over 4

=item Cookies

Interchange can issue cookies that contain the user session ID.
If the user returns the cookie, then they can be presented 
pages without accompanying session information.

=item URL re-writing

If the user doesn't want to use cookies for whatever reason, Interchange
...
Interchange will automatically do the right thing depending on whether the
user gives back a cookie.

Pages in the catalog served by Interchange running as a cgi-bin program generate a
special URL for every link. Here is an example of such a URL:

    http://www.minivend.com/cgi-bin/simple/browse1?id=WehUkATn&mv_pc=122
...
Internet address of the server hosting the Interchange catalog.
...
Name of the program to run -- this is Interchange's VLINK or TLINK
...
Interchange pages are written in regular HTML with extensions to
support catalog ordering. Interchange extensions look like:

    <A HREF="[href specials]">See our specials!</A>

or

	Count from one to three:<BR><BR>
	[loop list="1 2 3"]
		[loop-code]....<BR>
	[/loop]
...
The HTTPD server (Apache, Netscape Enterprise server, and Roxen
are examples of HTTP servers) receives a request for an Interchange page.

=item *

The server is already running as a daemon, and the request calls a small
C program that is I<named according to which catalog is being called.>
This program communicates with the Interchange program via a UNIX-
or INET-domain socket.
...
Interchange is normally free of charge, and is distributed under the GNU
...
must fulfill. See the file B<Copying> which came with your Interchange
...
However, the authors are always looking to improve Interchange and sometimes
...
Interchange mail list and hope that someone will help you.

=head2 Where to Download Interchange

The latest stable Interchange is available from:

	http://www.akopia.com/

=head2 Perl

You will need Perl version 5.005 or higher to run Interchange 4.x. Many
...
all reside there. If you use SQL, this is not normally a problem.

=head1 QUICK START

NOTE: See http://developer.akopia.com/ for the most recent installation
information.

Obtain, decompress and untar the distribution:

    gzip -dc interchange-4.xx.tar.gz | tar xvf -
...
Before installing, check the site where you obtained Interchange for any
...
or the B<Interchange software directory>.

Once you have installed the software, you will want to run the program
C<makecat> in the Interchange bin directory. It will build a demo catalog
which you can use as the base for your installation.

To install a catalog, you will eventually need to have write permission
on your CGI and HTML directories. The definition of these are typically
part of your HTTP server configuration; In Apache, the CGI directory is
set by B<ScriptAlias>; the HTML directory is set by B<DocumentRoot>.
...
Otherwise, Interchange should be installed at the completion of the
...
to do anything useful with Interchange until you have created a catalog.

You will want separate directories to hold the catalog pages and
databases. The C<makecat> program supplied with Interchange will make those
for you.
...
Any pages with Interchange elements/tags go in the directory set by the
I<PageDir> directive (the default is C<~/catalogs/catalog_name/pages>).
For the demos supplied with Interchange, this means that only one or a few pages
...
space, you should disable all access to your Interchange catalog directory
...
Interchange. If you can set file permissions such that files will not be
...
Interchange pages are NOT in normal HTML space. They are contained in the
...
Directory containing database source files, including the special Interchange
...
The supplied C<makecat> script, which is in the Interchange program directory
...
Interchange. You can also look for older demo catalogs (mostly for ideas)
...
The global C<minivend.cfg> file is located in the main Interchange directory,
...
The script names which, when containing an Interchange link program, will
...
Interchange is a complex program, and needs the services of other complex
programs (i.e. web servers and relational databases) to work. When there
is a problem, it is not always Interchange. It may have to do with Perl or
your HTTP server setup. In fact, in the over four years of Interchange's
...
or configured Perl and has nothing to do with Interchange. Contact your
...
to work, try running in INET mode.  Finally, see the Interchange FAQ at:

    http://www.minivend.com/minivend/faq/

Check the two error log files -- C<error.log> in the Interchange
...
You may subscribe to the Interchange users mail list by sending the
...
files are world-writable, the vlink program and the Interchange server
...
B<IMPORTANT NOTE:> The Interchange server should not run as the user
C<nobody>!
...
setup. Determined and thoughtful users almost always make Interchange work.

=head1 SETTING UP YOUR CATALOG

Interchange uses its own tags to implement catalog functions -- they
...
You will need to become familiar with the Interchange tags and
...
Items related to the item in the record. Using Interchange's C<[loop item]>
...
Interchange's demo catalog shows many of Interchange's features -- list
...
C<category>, and classify them for search by Interchange.
...
The demo catalogs supplied with Interchange are there to give you a starting
...
is not served by Interchange. If you do, they will lose their session (and items
in their shopping cart). Interchange will help in this regard, but you must
use its C<area> and C<page> tags.

If you are using frames, you must be careful to source all frame panes
containing Interchange links from an initial page served by Interchange. If you do not,
...
know to make the most basic of Interchange catalogs, one which displays
...
into the Interchange directory and your HTML directory.
...
of Interchange capabilities, though not nearly all of its features are used there.
...
on an HTML editor. Interchange can coexist with some editors; ones that
...
As stated before, Interchange catalogs are all about the database. The
...
Two fields; code and quantity. Interchange's demo will decrement the quantity
...
A long description of the product. If you use the Interchange internal database, the
...
=head2 MML -- Interchange Markup Language

Interchange functions are accessed via MML, the Interchange Markup Language.
The pages in the catalog are in HTML, but use MML tags to provide access
to Interchange's functions. MML tags use C<[square brackets]> instead
of angular brackets. We will usually call them B<Interchange tags> or
...
You will normally not want to use regular hypertext links in Interchange
...
<IMG SRC="URL">. But since Interchange pages are served by a CGI program you
cannot use relative links.  Interchange has the capability of defining an
...
intermix standard HREF and Interchange page links without fear of losing
...
If the user has sent a cookie to Interchange (meaning the B<second> page
...
When you send a form to Interchange, it reads the values and places
...
Interchange filter types to the value. See I<Filters>.

When the value is returned, any Interchange tags present in the value will be
escaped. This prevents users from entering Interchange tags in form values, which
...
Interchange is very complicated but very powerful. If you have read and
...
built Interchange catalogs -- you can too. If you feel it is beyond you,
then we would suggest engaging a competent consultant. Good luck!

=head1 DATABASES

Interchange, as with most powerful shopping cart programs, is all about
databases.

NOTE: No other database besides Interchange's internal one is needed.
You may find that keeping your database in an SQL manager makes
it easier to integrate Interchange with other tools. Interchange is fully
...
Interchange implements the database in GDBM, DB_File, SQL,
or in-memory format. In most cases, these should operate the same when
called by Interchange's access methods.

=head2 A bit about text source files

Interchange reads TAB-delimited text files to obtain your data. However,
...
Interchange will connect to the SQL database using the
...
Interchange will place the data in as in the columns; you must take
...
Either one is a reference to the B<key> for the database. In Interchange
...
Interchange's import, export, and search facilities will work much better
with that practice.

=item field, column

This is a column of the database. One of the columns is always the key --
Interchange prefers that the key be the first column. Field is an interchangeable
reference.

=item table, database

A table in the database. Because of the evolution of Interchange from a
...
where a database contains a series of tables.  Interchange cannot create
...
Interchange uses one mandatory database, which is referred to as the products
...
Interchange also has a two of standard but optional databases,
...
See I<Interchange Database Capability>for details on the format.
...
names of the C<[item-field]> tags in your catalog pages, or the Interchange
...
list of valid Interchange database identifiers, those databases will be
...
file arbitrary.csv, and the identifier it will be accessed under in Interchange is
...
=head2 Interchange built-in database support

If you specify one of the first 6 types, the database will automatically
be built in the default Interchange DB style. You can specify the type with
...
you restart the Interchange server, it will re-import all in-memory databases
for every catalog.
...
Microsoft Excel is a widely-used tool to maintain Interchange databases,
...
second field. If you specify the C<INDEX field> modifier Interchange will
...
reconfiguration or Interchange restart. The big advantage of using MEMORY
...
once in normal operation. Also see I<NoImport> and I<NoImportExternal>
for a way to guarantee that the table will never be imported.
...
of an order or other operation. Use Interchange's C<[import ...]> tag.
...
of the valid Interchange delimiter types, with the default being TAB.
...
Interchange, you can create an I<AdminPage> (or any page, for that matter)
...
on the Interchange server instance, you may define a database in 
...
In most cases, Interchange cannot perform administrative functions like
...
blank database and the permission to read and write it, Interchange can
...
and N is the length (if needed).  Most Interchange fields should be of the
...
lines as needed. This is not a DBI parameter, it is specific to Interchange.

=item NAME

A space-separated field of column names for a table.  Normally not used --
Interchange should resolve the column names properly upon query.  Set this if
...
it is specific to Interchange. All columns must be listed, in order of their
position in the table.

=item NUMERIC

Tells Interchange to not quote values for this field; allows numeric data
...
A Interchange delimiter type - one of TAB,CSV,PIPE,%%,LINE or the
...
make the first column the key for any Interchange database.
...
When importing a file for SQL, Interchange by default uses the
...
B<WARNING:> If Interchange has write permission on the products database,
...
always back up your database before enabling it for use by Interchange.

If you frequently use SQL (or LDAP) databases and always set them up
without the import assistance of Interchange, you can use the
catalog.cfg setting:

	NoImportExternal  Yes

See I<NoExportExternal>.
...
Most of the tests use Perl code, but Interchange uses the Safe.pm module
...
That will cause Interchange to look in the user form value array and return
...
Positional parameters cannot be derived from other Interchange tags; for
...
case and Interchange tags in lower case so pages are easier to read, but you
...
This is provided to allow HTML editors to work with Interchange if they wish.
...
tag will not be re-interpreted for Interchange tag constructs.
...
The C<[data ...]> and C<[field ...]> tags access elements of Interchange databases.
...
key Interchange conditions like whether an item will be ordered on a separate
line.
...
searching an Interchange DBM database, don't forget to use C<st=db>
(mv_searchtype).
...
The Interchange session variables. Of particular interest are I<logged_in>,
...
Many Interchange functions can be controlled or specified with C<[tag ...][/tag]>
pairs.
...
Content-Description. For example
   ...
   [tag op=mime description="My Plain Text"]
   	Your message here.
   [/tag]
...
Allows MIME attachments to be included -- typically with PGP-encrypted
credit card numbers. See the demo page etc/report for an example.

Named parameters:

	[tag op=mime
		 arg="reset|boundary|id|header"
		 type="media/type"
...
You can set the MIME media type like C<application/pgp-encrypted> or 
C<application/x-pdf> or C<application/octet-stream>.

=item arg

THere are four subsidiary operations which can be done instead
of the default encapsulation of a message. None are normally used;
they are only needed if you wish to hand-craft a MIME message including
the email message headers.

	boundary

	Returns a MIME message boundary with unique string keyed on
	session ID, page count, and time. Not normally needed; if there are
	nultiple parts then boundaries will be automatically generated.

	id

	Returns the current MIME id used in the boundary. Not normally needed.

    header 

	Returns a MIME header suitable for inclusion in a message. Affected 
	by the attach_only parameter.

=item attach_only

Interchange sends messages in C<multipart/alternative> encoding. If the
C<attach_only> parameter is set true (non-zero, non-empty) then the
MULTIPART/MIXED type is used.
...
Comments out Interchange tags (and anything else) from a page. The 
...
If you define the above UserTag and put put this on an Interchange page
...
C<mvasp> tags are the tags Interchange defines to C<Gobble>.
...
Interchange will usually do the right thing.
...
B<NOTE:> New to Interchange 3.04.
...
Interchange expects the C<banner> field to contains the banner text. It
...
that the proper query can be made. Here is the example from Interchange's 
demo:
...
You can put Interchange tags in the category parameter if you
wish:
...
If you have a colon-separated category, Interchange will select the most
...
(C<ProductFiles>), Interchange will use the special page descriptor C<flypage>
...
a link to a special page keyed on the item, Interchange will attempt to
...
field which contains a valid Interchange page name (relative to the catalog
...
By convention Interchange defines this as the page C<results>.
...
Interchange allows you to debug your page HTML with an external page
...
Interchange supports file upload with the C<multipart/form-data>
...
Interchange treats some form fields specially, to link to the search engine
...
The variable sets can contain and be generated by most Interchange tags --
the profile is interpolated for Interchange tags before being used. This
...
details -- except that Interchange form actions are defined instead
...
Interchange has a facility for greater control over the display of form
...
The Interchange companion application C<MiniMate> uses this facility extensively.
...
Interchange filters. For example, if C<mv_data_filter_foo> is set
...
The Interchange action B<set> causes the update. Here is an pair of example forms.
...
C<scan>. The search engine uses many special Interchange tags and variables.

If the search is implemented in a link or a form, it will always
display formatted results on I<the results page>, an Interchange page
that uses some combination of the C<[search-region]>, C<[search-list]>,
C<[more-list]>, C<[more]>, and other Interchange tags to format and display
...
Interchange will search the C<products.txt> file for the string entered
...
be located. If you installed Interchange in the default
...
If the Glimpse executable is not found at Interchange startup, the Glimpse
...
index file is present, Interchange will set the options:
...
Interchange will do a complete range of tests on individual columns in the
...
having to do this, use Interchange's option array feature:
...
there is an C<inventory> table as in some of the Interchange demo catalogs.
...
Interchange allows you to pass a search in a URL, as shown before. Just
...
B<IMPORTANT NOTE:> An incompatibility in earlier Interchange catalogs
...
normal variable in an Interchange profile. Actually C<mv_value> is a misnomer,
...
These correspond to the Interchange search variables that can
...
I<SearchProfile> directive. (You must reconfig the catalog for Interchange
...
for passing to Interchange. It is also necessary that you copy the hidden
...
in the Interchange demonstrations.)
...
Normally Interchange will return the first page of a search. If you set
...
Normally Interchange searches all lines of an index/product file but the
...
directive -- it is cleared by Interchange if there is no overflow.
...
C<[search-region][/search-region]> pair. This is a change from Interchange 3.
...
The standard set of Interchange iterative MML tags are available. They are
...
tell Interchange to look for the assignments.
...
Calls the search with the proper URL, including Interchange session tags. Used
as the ACTION value for the search form.

=item [process-target frame]

Calls the search with the proper URL, including Interchange session tags.
...
Interchange will directly interface with Perl libraries provided by
CyberCash, Inc. This allows direct card billing at the time the order
is placed. Interchange supports CyberCash 3; CyberCash 2 is no longer
in use.
...
The CyberCash Perl library module must be available to Interchange -- if you
cannot install it in the main Perl library then the Interchange software
directory I<lib/> will suffice.  If it is found at Interchange startup,
a message will be displayed.

Interchange only will charge CyberCash in the final phase of the order
...
the same as on the standard Interchange demo order pages:
...
will never be written to the user session, at least by Interchange
...
Interchange supports CyberCash 3 with the CCMckDirectLib3_2.pm
...
=head2 Common problems with CyberCash

There are several common problems with integrating CyberCash with
Interchange.

=over 4

=item Executables not found

It is not enough to just copy the CyberCash modules to the Interchange
library directory. You must make sure that the executables hard-coded
in to the module are at the correct paths. Unfortunately, CyberCash
places these executables (usually C<MCKdecrypt>, C<MCKencrypt>, and
C<computeMD5hash>) in a different directory than the Perl library 
directories and provides no method for relocating them. 

Make sure that all references to those executables 

=item STDERR not available

CyberCash opens a STDERR handle at compile time, so further calls to
its error logging will cause errors. This is benign, because Interchange
will report normal errors, but it can give a misleading error condition
if the executables are not found or the merchant key is wrong.
...
Interchange has a powerful custom shipping facility that performs
...
Whether shipping is based on weight, quantity, price, etc. Valid Interchange
...
Perl subroutine or Interchange C<[calc][item-list] ... [/item-list][/calc]>
...
If the character C<f> is the first, Interchange will first interpret the
text for any Interchange tags and then interpret the result as a formula.
It is really as Perl code; so you may reference the entire set of Interchange
...
returned from an Interchange tag, then that number is used directly.
...
available to Interchange in TAB-delimited format. (As of March 1997, you
...
www.ups.com) and place them into an Interchange database. That database, its
...
"c", it configures another lookup zone, which must be lettered from C<A to Z>.
It takes the format:
...
the match on weight must be exact, and Interchange rounds the weight up to the
next even unit.
...
The parameters are interpreted for Interchange tags before being parsed.
...
The I<rate database> is a standard Interchange database. For US shippers,
...
To adapt other shipper zone files to Interchange's lookup, you will need
...
Interchange has a user database function which allows customers to save any
...
an option. A couple of the fields are reserved for Interchange's use.
...
If you don't keep critical information and don't do Interchange
...
You can implement a simple access control scheme with the Interchange
...
The database entry should contain the complete Interchange-style
...
Interchange allows the entry of orders into a system via one of several
...
complete. Therefore it is the perfect place to put Interchange tags that
...
The above can be easily done with Interchange's C<[import ....]> tag
...
Interchange order email sequence will not be performed.
...
B<IMPORTANT NOTE:>  Interchange attempts to perform operations securely,
...
Interchange uses the I<SecureURL> directive to set the base URL for secure
...
With Interchange's I<GlobalSub> capability, very complex add-on schemes
...
In addition, you can create a file in any Interchange page subdirectory
...
of built in POSIX support and Interchange's I<Locale> directive.
...
HTMLsuffix is set to the locale, Interchange first looks for a page with
...
translation for all intents and purposes. Interchange tags and Variable
...
Interchange will format a currency number based on a "picture" given to it. The
basic form is:
...
If you set Locale keys corresponding to some Interchange C<catalog.cfg> directives,
...
Interchange has the capability to read its locale information from a database,
named with the I<LocaleDatabase> directive. The database can be of any
valid Interchange type. The locales are in columns, and the keys are in
...
Interchange has a powerful paradigm for extending and enhancing its functionality.
...
Many of the internal Interchange routines can be accessed by the savvy programmer
who reads the source and finds the entry points. Also, many internal Interchange
...
update the Interchange code in most cases while still keeping your hack.
...
the Interchange daemon's permissions! See I<GlobalSub> and I<UserTag>
for ways to make external files and programs available to Interchange.
...
Interchange server runs for different companies/user ids. Be careful! Security
...
But for Interchange databases, you can access them as long as you pre-open
...
Any Interchange tag (except ones using SQL) can be accessed via the $Tag
...
there. The subroutine will be able to modify any variable in Interchange,
and will be able to write to read and write any file that the Interchange
...
plain HTML that will be placed unchanged on the page. Interchange variables
and [L][/L] and [LC][/LC] areas will still be inserted, but any Interchange
tags will not be.
...
You can, however, use the Interchange equivalent routines:
...
a liberal dose of the Interchange source code, but for simple things you can
use something like:
...
It is the same as Interchange's C<db> searchtype. Options are specified
...
A reference to the I<current> shopping cart. Unless you have used an Interchange
...
Using the $Tag object, you can access any Interchange tag including
user-defined tags.
...
any named parameters as you would specify in an Interchange tag. The calls
...
Interchange can and usually does run multiple catalogs on the
same server.
...
are relative to the catalog directory (or Interchange software directory,
if in the main minivend.cfg file).

You can also use a type of "here document" to specify Interchange directives,
...
is the default location of all of the Interchange program, configuration,
special, and library files. Unless changed in the call to C<minivend>,
the main Interchange server configuration file will be minivend.cfg in the
VendRoot directory.

The directives defined in minivend.cfg affect the entire Interchange
server and all catalogs running under it; though you may run multiple
Interchange servers on the same machine with totally independent operation.
...
Specifies a catalog that can run using this Interchange server.
...
Tells Interchange to dump the structure of catalogs and the minivend server
...
You can override the default Interchange actions if you desire.
...
How often, in seconds, the Interchange server will "wake up"
...
This will be executed with the user ID that Interchange runs
...
when Interchange issues a cookie it does so for the base domain. This will allow
...
Whether Interchange C<[file ...]> and other tags can read any file on the system
(that is readable by the user id running the Interchange daemon).  The default is
...
If non-zero, enables a check of running Interchange processes during the
...
Default is B<0>, disabling the check.
...
The file which will contain the Interchange server process ID -- so that it
...
This file must be writable by the Interchange server user ID, of course.
...
'/usr/lib/sendmail'. If it is not found at startup, Interchange
...
Must be in a directory where the Interchange user has write permission.
...
set it on the command line as well:
    ...
    bin/interchange -r SocketFile=/tmp/mv.socket
...
Allows you to set up syslog(8) error logging for Interchange.
...
by the Interchange server. Default is 7786>
...
running on the server I<www.akopia.com>, named C<simple>, C<sample>,
...
document notation in interchange.cfg.
...
all Interchange catalogs running on the server. Otherwise this is
...
Interchange writes it. For instance, if you wanted the variable mv_session_id
...
time Interchange is run. Setting it in minivend.cfg is probably better for
clarity.
...
the configurable parameters for Interchange -- each is independent
...
<<HERE document, but <&HERE is the notation. See C<Interchange Programming>
for details.
...
points on array values is not recommended -- most Interchange subroutines
...
KEY is the index into the hash. An example of HASH type Interchange configuration
values 
...
These are the standard supplied actions for Interchange. You can overwrite
...
by Interchange, not even the special C<missing> page. You can of course generate
...
Allows arbitrary HREF links to be placed in Interchange pages, while
...
C<mv_credit_card_exp_year> are looked at.  It is set by Interchange when the card
...
present.  It is set by Interchange when the card validation returns, if not
previously set.
...
Set by Interchange to indicate the error if the card does not validate properly.
The error message is not too enlightening if validation is the problem.
 

=item C<mv_credit_card_force>

Set this value to 1 to force Interchange to encrypt the card despite its idea of
...
Set this value to 1 to cause Interchange encrypt only the card number and not
accompany it with the expiration date and card type.

=item C<mv_credit_card_info>

Set by Interchange to the encrypted card information if the card validates
...
Set by Interchange to true, or 1, if the the card validates properly. Set to 0
otherwise.

=back

PGP is recommended as the encryption program, though you should remember
that US commercial organizations may require a license for RSA. Interchange
...
this to "Yes" will display the error returned from Interchange in case something
is wrong with embedded Perl programs, tags, or (horrors!) Interchange itself.
...
If PGP is the encryption program (Interchange determines this by searching
...
This is where Interchange will write its runtime errors for THIS CATALOG
ONLY. It can be shared with other catalogs or the main Interchange error
...
Relocates images based in a particular directory to another for Interchange
use -- operates I<after> ImageDir. Useful for editing Interchange pages
...
Set to the Interchange database I<identifier> of a table that contains
...
table(s) will never be subject to import. Useful for external databases
(like SQL or LDAP ones), or databases that will *never* change. To exclude
all external databases at once, use the NoImportExternal directive.
...
You might enable this directive after your initial catalog build.
...
The location of the offline database files for use with the Interchange
...
An C<onfly> tag is provided by Interchange -- see <On-the-fly> ordering.
...
Interchange server, or defined by the environment variable PGPPATH, and the
...
By default, only the user account that Interchange runs under (as set by the
setuid permission on vlink) can read and write files created by Interchange.
...
supplied with Interchange -- it is typically received from a third party by
...
the MV_SESSION_ID cookie). The ones used in Interchange by default are
...
special Interchange tag syntax to provide conditional setting of search
parameters.
...
If set to C<none>, no mail can be sent by standard Interchange facilities.

The default is the value in C<INTERCHANGE.CFG> and varies depending on
operating system.

=head2 SeparateItems

Changes the default when ordering an item vian Interchange to allowing multiple
...
very possible for multiple Interchange servers to serve the same catalogs.
If you are serving a massively busy store, multiple parallel Interchange
...
The name of the Interchange database to be used for sessions if DBI
...
A yes/no directive.
Tells Interchange to try and build all pages in the catalog statically
...
for static pages. The user ID executing Interchange must have write permission
...
when employing the static page-building capability of Interchange. Not necessary
if using StaticAll.
...
in pages built with the static page-building capability of Interchange.
...
Interchange page directory -- if set to .htm the pages must be named with that
extension.
...
or [mvasp] embedded perl languages. Use the "here document" capability of Interchange
...
Interchange databases are similarly affected, though the methodology is
different.
...
in any Interchange page, and can contain any valid Interchange tags except
global variables.
...
be used to set Interchange variable values. For example, a database
defined as:
...
hash-oriented Interchange directive, such as I<SpecialPage>, I<ScratchDefault>,
...
might have multiple users sharing an Interchange server. In
...
main directory where you installed Interchange. If you made /home/minivend/mvend
your Interchange base directory, the start command would be
...
This is typically done to force Interchange to re-read its configuration.
...
network (LAN), is used for accessing an Interchange server.
...
or that the Interchange server runs on the same machine as any SSL-based
...
several forms of Interchange authentication.
preferably by HTTP basic authorization (see I<RemoteUser>).

You can reconfigure from the command line (as the Interchange user) with:

    VENDROOT/bin/minivend -reconfig <catalog>

It is easy to manually reconfigure a catalog if you are the administrator.
Interchange simply looks for a file C<etc/reconfig> (based in the Interchange
...
directories or file from growing too large.
...
This is less necessary with the default file-based sessions, but very
busy stores will find the session directory beginning to take lots of
file space.  The file names are stored in a two-level pseudo-hashed
directory structure, so up to 3364 directories and two

The C<expire> script accepts a C<-r> option which tells it to recover
lost disk space. This only applies to DBM sessions, which are rarely used.
The option is ignored if using the default file-based sessions.

On UNIX, you can add a crontab entry such as the following:

    # once a day at 4:40 am
    40 4 * * *    /usr/local/interchange/bin/expireall -r
...
When using DBM sessions on Windows or other operating systems which
don't fork(), you will need to stop the server before running C<expire>
...
An Interchange installation is complex, and requires quite a few distinct
steps. That is why there is an interactive configuration script that is
included with Interchange -- it merely does automatically what is described
...
The Interchange program, and its supporting libraries, should all
...
served through the Interchange cgi-bin program and contain nonstandard
...
permissions are complex and very important.  Please let the Interchange
configuration program do the work!

You will want a public WWW directory for in-line image graphic files.
Interchange does not serve the images, only the HTML tags calling them. A
...
use an I<absolute path> -- relative paths will not do. Interchange
...
Interchange requires a that a web server be installed on your system in
...
Interchange, by convention, names the link program the same name as the
...
or C</simple.cgi>. This SCRIPT_PATH can be used to determine which Interchange
...
C<vlink> is the provided facility for such communication with Interchange.
...
gets a bit tricky. Interchange comes with a program, C<makecat>, which
...
Interchange will perform the HTTP server function itself, talking directly
...
being served from the internal server, Interchange has a fixed SCRIPT_PATH
...
Interchange daemon. The vlink executable is normally made setuid to the
user account which runs Interchange, so that the UNIX-domain socket file
...
the configuration program. If the Interchange daemon is not running, either
...
Set this to the TCP port number that the Interchange server will monitor.
...
announcing that the Interchange server is not running. The default of 45
...
If you want Interchange to run under a different user account than your own,
...
Mike Heins, F<heins@akopia.com>.

=head1 ACKNOWLEDGEMENTS

First, to Andrew Wilcox for his 1995 Vend, which inspired Interchange. There are
still a few vestiges of his code in Interchange, and it would not have happened
without it and him. Thanks, Andrew.

There have been many who contributed to Interchange's development, especially with
