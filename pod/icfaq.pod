...
=head1 How does Interchange work?
...
B<NOTE>: You cannot run the Interchange software as root.
...
root B<as long as your umask is set to 2 or 22>.
...
Interchange server. B<Important note:> The server should always be
...
=head2 Why isn't the above error more enlightening?
...
=head2 Can I run Interchange on the Macintosh or Windows?
...
B<NOTE>: Interchange does not support this configuration. You may be
...
=head2 I have a different secure server domain. Why does the shopping cart get dropped?
...
=head2 My images aren't there on the secure server!!!
...
=head2 My secure pages fail when the browser is MSIE.

MSIE has several SSL bugs, particularly in V5.01.  See the
Apache-SSL or mod_ssl FAQ. You can sometimes fix this with an
httpd.conf change:
...
You can run in INET mode with the tlink link program to prevent
those problems.
...
=head2 My entire home directory is in HTML document space.
...
whitespace). If your su(1) command has a -c option (as most System
...
=head2 I installed the Interchange RPM, and I can't restart.
...
=head2 Can I change the price based on size or color (or other attribute)?
...
=head2 How are simple product options structured?

Interchange has three types of options; I<simple>, I<matrix>, and
I<modular>. They are based on the options database table.
...
=head2 But what do these options do? Where do they live?

If you know Perl, you know what a hash reference is. An Interchange
shopping cart consists of an I<array of hash references>. If you dump
...
Any attribute besides the above is a product I<option> or I<modifier>,
...
will contain PGP or GPG's error output.
...
"pgpe". The key is set in EncryptKey. If you don't set a value for
...
set. NOTE: old values in EncryptProgram will still work, just are not
needed.
...
=head2 I am using SQL, and Interchange is slow ...
...
=head1 Using Interchange with Apache and SUEXEC
...
=head1 A friendly reminder

B<When in doubt, restart the server.> It won't take but a few seconds,
...
=head2 Locking down your system
...
other user ID, so CGI/PHP programs run by the web server can't get at
them.
...
[data table=foo col=bar key="[item-param something]"]) which is not
...
It is the default for MV DBM databases (i.e. you have to have a [flag
type=write table=foo] to write) but not for SQL.

The UI does the proper write flagging, so this should have no effect
on it.

AUTHORS NOTE: I should have made WRITE_CONTROL the default for SQL in
MV4, my bad. As long as MV3 compatibility was as poor as it turned out
...
production catalog, and disable the UI for the production catalog.
...
You could also use a different username to access the SQL data, and
make the production server username have read-only access.

You can use a different UID for the admin server iteration, and make
