...
parser does not replace ']-->' with ']' unless it also sees '<!--[' at
least once somewhere on the page. (This is a small parsing speed
optimization.)
...
In the named style you supply a parameter name=value pair just as most
HTML tags use:

    [value name="foo"]

The positional-style tag that accomplishes the same thing looks like
this:
...
the same as named parameters. However, there are a few situations
where you need to use named parameters:
...
parameter that you want to omit, e.g. omit the first parameter but
specify the second. The parser would have no way of knowing which is
which, so you just specify the second by name. This is rare, though,
because the first positional parameters are usually required for a
given tag anyway.
...
In any of the option building types, you can append the string
ranges and a special option processing will be done -- any option
matching the pattern [A-Za-z0-0]..[A-Za-z0-0] will be expanded into a
comma separated range between the bounds. The same behavior is
accomplished by passing the accessories tag option ranges. For
example:
...
The above applies to any of the option building types -- check,
combo, combo_move, labels, multiple, options, radio,
reverse_combo, and select. It will refuse to produce more than
5000 options -- that limit can be changed with Limit option_list N
in catalog.cfg, where N is an integer greater than 0.
...
option. See the example in form above; if empty=1 had been
...
block executes if the correspondingly labelled  try block fails.

You can also return a result based on the error message caught in the
try block with paired subtags, like S<this:>

   [error message]body text[/error message]

Note that this feature excises I<all> tag/endtag pairs if
interpolation is turned off, so the catch tag interpolates by
default.
...
Ignoring whitespace, the following would return division result if
successful, 0 on a division by zero, or an error S<message:>
...
The page content contained within the [catch I<label>][/catch]
block executes if the correspondingly labelled  try block fails.
The catch block executes in place on the page if triggered (I<i.e.>,
it does not return its result in place of the try block).
...
The error message to use in the special block will generally be part
of the entry the error generates in your error log.  For example, a
division by zero error generates something like the following in the
error log:
...
(note that trailing backslashes in the example indicate a continued
line).

I<label>

This is the label specifying the corresponding [try block. Defaults
to 'default'.
...
WARNING: This tag will not work under I<Safe>, i.e. in embedded Perl.
...
piece in turn. If a value returns true (in the Perl sense: non-zero,
non-blank) then subsequent pieces will be discarded without
interpolation.
...
you cannot use the '$Tag->goto()' Perl syntax. Note also that skipping
...
B<Technical note (Interchange 4.8): >This tag may not work properly if
...
This should match the name set in a [label] tag I<after> the goto tag
in the page (i.e., don't create loops).

I<if>

Condition for goto. If the argument to 'if' is true, the tag will
skip the text between the goto and <label>.  Note that the tag
...
the label with [loop-param label]. If the size data for SKU
TS-007 was set to the string S=Small, M=Medium, L=Large, XL=Extra
...
option: in product options. If the size data for SKU TS-007 was set
...
Builds HTML widgets as defined in the options table for selecting
...
off, and I<Safe> will block you from turning them on, since it blocks
...
If you do not do this, your code will fail with a runtime I<Safe>
...
I<Safe> objects (including database handles) may persist within a
...
$Tag->accessories call will fail with a I<Safe> error unless you also
...
under the I<Safe> module (see I<tables> above).
...
Note the mismatched quotes in the expansion.  Your surrounding HTML
should supply the containing quotes, like S<this:>
...
code is secured by the I<Safe.pm> module), you must be sure to set the
...
If you do not set a I<type>, the tag will return a reference to an
array of array references, since the default with no I<type> is
arrayref="".
...
GlobalSub or global UserTag, if you call query() in list context and
want the three references (i.e., results, column hash and column
array), then you need to set the 'I<wantarray>=1' attribute in the
query() call.  See the I<wantarray> attribute.
...
value for those types that would return references if $Tag->query
...
enable certain ITL tags. If you set allow by default, you must be
...
Returns the shipping description for the specified shipping B<mode>. 
See the I<Shipping> section of the Database documentation.  See also
shipping.asc database for shipping modes.

S<Alias:> B<shipping_description>

The two tags below are identical in S<operation:>
...
MD5), and that file name is checked in the directory tmp/auto-timed
...
Implies login=1, but will still abort if no session ID cookie has
...
on the page (I<i.e.>, it does not replace the output of the try
block).
...
The try tag will also place a result in the $Session object.  For
example, the following returns the 'Illegal division by zero...' error
message if it S<occurs:>
...
The $Session->{try}{divide} object will be set to the empty string
('') if there was no error, or it will contain the error message if
there was an error.

The [perl] and [calc] tags also set
$Session->{try}->{I<active_label>} on errors.
...
I<Safe.pm> module with warnings disabled. Unlike [perl] tags,
...
usertag directory in the Interchange software directory. Some of
these are used by the foundation catalog or its administrative
interface.

=head2 bar_button

Displays content based on current page. Could be used for building
e.g. menu bars.
...
I<userdb>, I<load_cart>, I<save_cart> and pages
...
I<email_raw> and etc/mail_receipt, pages/process_return.html,
...
an email message. The string <no subject> will be substituted if this
parameter is omitted.

to

Valid email address(es) of the recipient(s). This parameter is
required.

=head2 email_raw

This tag takes a raw email message, B<including headers>, and uses the
...
Uses the LWP libraries (LWP::Simple) to fetch a URL and returns the
contents.
...
I<userdb>, I<delete_cart>, I<save_cart> and pages
templates/components/saved_carts_list_small, pages/saved_carts.html
for more examples.
...
I<userdb>, I<delete_cart>, I<load_cart> and pages
templates/components/saved_carts_list_small, pages/saved_carts.html
for more examples.
...
with lower case <td> elements, then set the embed tag and make the
cells you wish to organize be <TD> elements. To switch that sense, and
...
defaults to variable HEADERTEXT or when its not present to white.
...
This is a quick and dirty tag that generates XML tags based upon one
...
name, and keys as separate XML container tags. If the parameter "that
is equal to the type" is given, only those fields will be shown.
...
Add option to select tables. E.g. dump_tables="products cat area"
and/or

=item 2.

Add option to select an output file. E.g. dump_file="tabledump.XML".
...
B<Note: >Field name list B<must> be tab delimited.
...
The toplevel tag name to use. Defaults to "table" for the 'dbdump
mode' and delimited type, and "session" for the other.
...
corresponding attributes. Defaults to '[\s,]+' (one or more whitespace
or comma).
...
the HTML comment delimiters and the tag, like this, '<!--  [tagname] 
-->'.
...
body text. Hence, the I<prefix>-tags are not handled recursively.

=item *

Some tags are interpreted in the lib/Vend/Parse.pm:start routine. You
cannot call them with the '$Tag->I<tagname>' syntax. They S<are:>
...
Check for I<prefix>_B<line> and prepare for it if present (does not
process I<prefix>-B<line> at this time)
...
B<if>-I<prefix>-B<modifier>  processed (B<if>-I<prefix>-B<param> and
B<if>-I<prefix>-B<modifier> are functionally identical except for
parse order)
...
I<prefix>-B<code>  processed again differently (operating on new
instances of I<prefix>-B<code> in output of above?)
