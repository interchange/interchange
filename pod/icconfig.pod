...
variety of frequently asked questions. The Interchange program must be
installed before continuing. For more information on the Installation
process, see the Installation (icinstall) manual.
...
catalog. Refer to the Reference Tags (ictags) manual for more
...
=head2 ITL - Interchange Tag Language

Interchange functions are accessed via ITL, the Interchange Tag
Language. The pages in the catalog are in HTML, but use ITL to provide
access to Interchange's functions. ITL tags are delimited by C<[square
brackets]> instead of angle brackets. They are usually called
Interchange tags or just tags. They are similar to HTML in syntax in
that there are both I<standalone> and I<container> tags. A standalone
tag has no ending element. For example:
...
The optional C<form> argument is used to encode a form in the link.
       ...
       <A HREF="[area                mv_order_item=99-102
               mv_order_size=L
               mv_order_quantity=1
               mv_separate_items=1
               mv_todo=refresh"
...
initial page:
    ...
    [set mv_no_session_id]1    [set mv_no_count]1[/set]
...
the item-code/SKU and use the C<refresh> action:
 ...
 <FORM ACTION="[process-target]"<INPUT TYPE=hidden  NAME="mv_todo"        VALUE="refresh">
...
places them in the user session. The following form:
 ...
 <FORM ACTION="[area index]"<INPUT TYPE=hidden NAME=mv_action VALUE=return>
...
Example:
    ...
    [perl tables="products pri        $key = $Values->{sku};
...
C<__>C<DescriptionField__>, which would by default be C<description>.
Array variables have a C<_N> added, where C<_N> is the ordinal index,
i.e. C<ProductFiles> becomes C<__>C<ProductFiles_0__>,
C<__>C<ProductFiles_1__>, etc. Hash variables have a _KEY added, i.e. 
C<SpecialPage> becomes C<__>C<SpecialPage_missing__>,
C<__>C<SpecialPage_violation>, etc. Doesn't handle hash keys that have
...
Writing can be controlled by catalog. See C<Database>.

=head2 DebugFile

Names a file, relative to the Interchange root directory, which should
store the output of C<logDebug> statements, and warnings if warnings
are enabled.
...
Increments a counter in C<ConfDir> for every access to the catalog.
The file is named C<hits.catalogname>, where C<catalogname> is the
short catalog identifier. A Yes/No directive, default is C<No>.
...
Sets the global C<Locale> for use in error messages. Normally set from
...
Names a file (or files) which contain C<OrderProfile> and
C<SearchProfile> settings that will apply for all catalogs.
...
C<FormAction> settings. Besides using Perl, Interchange tags may be
...
transaction ends. See C<Autoload>.
...
Forces a Perl module, global C<UserTag>, or C<GlobalSub> to be present
...
1. fcntl() supported in Perl 2. lock daemon running on server system
