...
implemented open source e-commerce platform. Interchange is designed
to build pages based on templates from a database. This document
discusses, in detail, how to build forms and report templates with
Interchange's tag language, ITL (Interchange Tag Language). A number
of frequently asked questions are also discussed.
...
results. "Generate definition" will display an ITL tag which can be
...
tag language with over 80 different functions, dubbed ITL, for
Interchange Tag Language. It allows access to and control over any of
an unlimited number of database tables, multiple shopping carts, user
...
=head1 VARIABLE REPLACEMENT

Variable substitution is a simple and oft-used feature of Interchange
templates. Before anything else is done on a template, all variable
tokens are replaced by variable values. There are three types of
variable tokens:

C<__VARIABLENAME__> is replaced by the catalog variable called
VARIABLENAME.

C<@@VARIABLENAME@@> is replaced by the global variable called
VARIABLENAME.

C<@_VARIABLENAME_@> is replaced by the catalog variable VARIABLENAME
if it exists; otherwise, it is replaced by the global variable
VARIABLENAME.

See the C<icconfig> documentation for information on how to use the
C<Variable> configuration file directive to set global variables in
C<interchange.cfg> and catalog variables in C<catalog.cfg>.
...
[comment]            Insert comments in Interchange pages
  [/comment]
[col]                Used with [row] -- rudimentary text tables for order
  [/col]             reports
...
[ecml]               Translate to-from Electronic Commerce Modeling Language
                     field
[else]               Defines else region for [if ...], [if-PREFIX-field ..]
  [/else]            and others
[elsif]              Defines elsif region for [if ...] (not available for
  [/elsif]           [if-PREFIX-field]
...
[PREFIX-price]       Display price of item with any discounts/price breaks/
                     adjustments
...
[row]                Used with [col] -- rudimentary text tables for order
  [/row]             reports
...
An order profile would be set with:
  ...
  [set che  name=required You must give us your name.
...
Checks to see if a discount is present for an item.
    ...
    [if discount 9    Item is discounted.
    [/if]
