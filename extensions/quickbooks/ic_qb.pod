...
=head2 Compatibility Modifications

Quickbooks versions 6.0 and 99 may have a problem with item names that are longer
than 30 characters.  The current version limits the item names to 45 characters 
(known to work on Quickbooks 2000 and later version), but can be easily changed in
the etc/trans_quickbooks file.  Please report any compatibility problems to the 
